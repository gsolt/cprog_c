...
* V4.0	2008.07.21	Zsolt Gergely	Idõbélyeg fogadás MOSCAD állomás esetén 	*
  v4.1     2014.04.16 Gergely Zsolt	Vételi számláló nullázása nap végén
   ...
   for (nI=0;nI<sCP.nRtuNum && nI<MAX_RTU;nI++)
   {
   	for (nJ=sCP.sCPR[nI].nDCStart;nJ<sCP.sCPR[nI].nDCStart+sCP.sCPR[nI].nDCNum;nJ++)
   	{
   		...
   		if (p_col_DCAct[nJ-nOffset]>0)
   		{
			...
			if ( (nI!=87) && (nI!=88)  && (nI!=76) && (sTAL[nI].nLeagNum<9))
...
ReteszParancsokKezdoCim = 1110;	/* DC5, 110 */																		/**/
																													/**/
ReteszesTMOKNum = 52;					/* Ennyi reteszfeltételes TMOK van az adott front-endben*/					/**/	
...
TMOK_ID[0][0] =1251;						/* TMOK azonosítója a kmenõ táviratban = DP offset */								/**/															
...
TMOK_ID[1][0] =1252;						/* TMOK azonosítója a kimenõ táviratban = DP offset */								/**/															
...
TMOK_ID[2][0] =1251;						/* TMOK azonosítója a kimenõ táviratban = DP offset */								/**/															
...
TMOK_ID[3][0] =1252;						/* TMOK azonosítója a kimenõ táviratban = DP offset */								/**/															
...
/* 4. TMOK: 70-27 RTU: Ostffyasszonyfa, Kemenesmagosi biogáz, Front end E ,Vönöck szélerõmû -----------------------*/												/**/																													/**/
...
TMOK_ID[5][0] =1253;						/* TMOK azonosítója a kimenõ táviratban = DP offset */								               /**/															
...
/* 7. TMOK: 34-26 RTU: Kincsesbánya 03-100 PV erõmû, Front end G  -----------------------*/											     /**/																													/**/
TMOKAllasjelzesOffsetek[7] = 564; 		/* Az állásjelzés offsete a DP adatbázisban */								               /**/
TMOK_ID[7][0] =1258;						/* TMOK azonosítója a kimenõ táviratban = DP offset */								               /**/															
ReteszesRTUIndex[7][0] = 323;			/* G front end */															                                 /**/
...
/* 8. TMOK: 32-14 RTU: TMOK 32607, 32608, 32609 Front end G  -----------------------*/											     /**/																													/**/
TMOKAllasjelzesOffsetek[8] = 363; 		/* Az állásjelzés offsete a DP adatbázisban */								               /**/
TMOK_ID[8][0] =1263;						/* TMOK azonosítója a kimenõ táviratban = DP offset */								               /**/															
ReteszesRTUIndex[8][0] = 323;			/* G front end */															                                 /**/
...
/* 9. TMOK: 32-61 RTU: TMOK 32607, 32608, 32609 Front end G  -----------------------*/											     /**/																													/**/
TMOKAllasjelzesOffsetek[9] = 47; 		/* Az állásjelzés offsete a DP adatbázisban */								               /**/
TMOK_ID[9][0] =1264;						/* TMOK azonosítója a kimenõ táviratban = DP offset */								               /**/															
ReteszesRTUIndex[9][0] = 323;			/* G front end */															                                 /**/
...
/* 10. TMOK: 32-65 RTU: TMOK 32607, 32608, 32609 Front end G  -----------------------*/											     /**/																													/**/
TMOKAllasjelzesOffsetek[10] = 698; 		/* Az állásjelzés offsete a DP adatbázisban */								               /**/
TMOK_ID[10][0] =1265;						/* TMOK azonosítója a kimenõ táviratban = DP offset */								               /**/															
ReteszesRTUIndex[10][0] = 323;			/* G front end */															                                 /**/
...
/* 11. TMOK: 32-24 RTU: A redundancia -> Alcsútdoboz 071-11 PV erõmû  -----------------------*/								/* Tesztelve */
TMOKAllasjelzesOffsetek[11] = 52; 		/* Az állásjelzés offsete a DP adatbázisban */								/**/
TMOK_ID[11][0] =1;   						/* TMOK azonosítója a táviratban = DP offset */								/**/															
ReteszesRTUIndex[11][0] = 324;			/* A redundancia */															/**/
...
nReteszOffset[11] = 11;             /* DC parancs és DP állapot offsete, ha tartozik hozzá DC parancs*/
                   
/* 12. TMOK: 70-34 RTU: G front end -> Celldömölk 2912/17 PV erõmû  -----------------------*/								/* Tesztelve */
TMOKAllasjelzesOffsetek[12] = 684; 		/* Az állásjelzés offsete a DP adatbázisban */								/**/
TMOK_ID[12][0] =1266;   						/* TMOK azonosítója a táviratban = DP offset */								/**/															
ReteszesRTUIndex[12][0] = 323;			/* G front end */															/**/
...
nReteszOffset[12] = 12;             /* DC parancs és DP állapot offsete, ha tartozik hozzá DC parancs*/
                   
/* 13. TMOK: 14-15 RTU: C front end -> J02-03  -----------------------*/								/*  */
TMOKAllasjelzesOffsetek[13] = 600; 		/* Az állásjelzés offsete a DP adatbázisban */								/**/
TMOK_ID[13][0] =600;   						/* TMOK azonosítója a táviratban = DP offset */								/**/															
ReteszesRTUIndex[13][0] = 170;			/* J02-03 */															/**/
...
nReteszOffset[13] = 13;             /* DC parancs és DP állapot offsete, ha tartozik hozzá DC parancs*/

/* 14. TMOK: 14-99 RTU: C front end -> J02-03, J04-05  -----------------------*/								/*  */
TMOKAllasjelzesOffsetek[14] = 43; 		/* Az állásjelzés offsete a DP adatbázisban */								/**/

TMOK_ID[14][0] =43;   						/* TMOK azonosítója a táviratban = DP offset */								/**/															
...
/* 16. TMOK: 42-50 RTU: E front end -> Szentgotthárd, vízierõmû  -----------------------*/								/*  */
TMOKAllasjelzesOffsetek[16] = 437; 		/* Az állásjelzés offsete a DP adatbázisban */								/**/
TMOK_ID[16][0] =1257;   						/* TMOK azonosítója a táviratban = DP offset */								/**/															
ReteszesRTUIndex[16][0] = 322;			/* E Front end */															/**/
...
nReteszOffset[16] = 16;             /* DC parancs és DP állapot offsete, ha tartozik hozzá DC parancs*/

/* 17. TMOK: 71-39 A redundancia, RTU: H front end -> Magyargencs, 184hrsz. , Vönöck szélerõmû -----------------------*/								/*  */
TMOKAllasjelzesOffsetek[17] = 74; 		/* Az állásjelzés offsete a DP adatbázisban */								/**/
...
nReteszOffset[17] = 17;             /* DC parancs és DP állapot offsete, ha tartozik hozzá DC parancs*/

/* 18. TMOK: 82-79 G front end, RTU:  Oroszlány 4700-9 PV erõmû  -----------------------*/								/*  */
TMOKAllasjelzesOffsetek[18] = 347; 		/* Az állásjelzés offsete a DP adatbázisban */								/**/
TMOK_ID[18][0] =1270;   						/* TMOK azonosítója a táviratban = DP offset */								/**/															
ReteszesRTUIndex[18][0] = 323;			/* G front end */															/**/
...
/* 21. TMOK: 30-80 RTU: TMOK 36601								-----------------------  */								/**/
TMOKAllasjelzesOffsetek[21] = 17; 		/* Az állásjelzés offsete a DP adatbázisban */								/**/
TMOK_ID[21][0] =1274;						/* TMOK azonosítója a táviratban = DP offset */								/**/			
ReteszesRTUIndex[21][0] = 323;			/* G front end		 */															/**/
...
/* 22. TMOK: 32-49 RTU: TMOK 36601								-----------------------  */								/**/
TMOKAllasjelzesOffsetek[22] = 695; 		/* Az állásjelzés offsete a DP adatbázisban */								/**/
TMOK_ID[22][0] =1276;						/* TMOK azonosítója a táviratban = DP offset */								/**/			
ReteszesRTUIndex[22][0] = 323;			/* G front end		 */															/**/
...
/* 23. TMOK: 21-37 RTU: G: Annavölgy PV								-----------------------  */								/**/
TMOKAllasjelzesOffsetek[23] = 78; 		/* Az állásjelzés offsete a DP adatbázisban */								/**/
TMOK_ID[23][0] =1277;						/* TMOK azonosítója a táviratban = DP offset */								/**/			
ReteszesRTUIndex[23][0] = 323;			/* G front end		 */															/**/
...
nReteszOffset[23] = 21;             /* DC parancs és DP állapot offsete, ha tartozik hozzá DC parancs*/
                                                                                                                                                                      
/* 24. TMOK: 30-98 RTU: G: Tác PV								-----------------------  */								/**/
TMOKAllasjelzesOffsetek[24] = 13; 		/* Az állásjelzés offsete a DP adatbázisban */								/**/
TMOK_ID[24][0] =1279;						/* TMOK azonosítója a táviratban = DP offset */								/**/			
ReteszesRTUIndex[24][0] = 323;			/* G front end		 */															/**/
...
nReteszOffset[24] = 22;             /* DC parancs és DP állapot offsete, ha tartozik hozzá DC parancs*/

/* 25. TMOK: 85-03 RTU: G: Almásfüztiõ 012 PV							-----------------------  */								/**/
TMOKAllasjelzesOffsetek[25] = 28; 		/* Az állásjelzés offsete a DP adatbázisban */								/**/
TMOK_ID[25][0] =1281;						/* TMOK azonosítója a táviratban = DP offset */								/**/			
ReteszesRTUIndex[25][0] = 323;			/* G front end		 */															/**/
...
nReteszOffset[25] = 23;             /* DC parancs és DP állapot offsete, ha tartozik hozzá DC parancs*/
 
/* 26. TMOK: 34-04 A redundancia, RTU: H front end -> Bicske 041/8 OVIT PV erõmû, Csákvár 0179/1  -----------------------*/								/*  */
TMOKAllasjelzesOffsetek[26] = 99; 		/* Az állásjelzés offsete a DP adatbázisban */								/**/

TMOK_ID[26][0] =8;   						/* TMOK azonosítója a táviratban = DP offset */								/**/															
ReteszesRTUIndex[26][0] = 324;			/* A redundancia */															/**/

TMOK_ID[26][1] =1335;   						/* TMOK azonosítója a táviratban = DP offset */								/**/															
ReteszesRTUIndex[26][1] = 323;			/* G front end */															/**/
...
nReteszOffset[26] = 24;             /* DC parancs és DP állapot offsete, ha tartozik hozzá DC parancs*/

/* 27. TMOK: 45-11 RTU: G: Gyöngyösfalu PV							-----------------------  */								/**/
TMOKAllasjelzesOffsetek[27] = 165; 		/* Az állásjelzés offsete a DP adatbázisban */								/**/
TMOK_ID[27][0] =1290;						/* TMOK azonosítója a táviratban = DP offset */								/**/			
ReteszesRTUIndex[27][0] = 323;			/* G front end		 */															/**/
...
/* 28. TMOK: 92-03 RTU: G: Nova 2037/2 PV							-----------------------  */								/**/
...
/* 29. TMOK: 32-88 RTU: G: Videoton 3523/34 PV							-----------------------  */								/**/
TMOKAllasjelzesOffsetek[29] = 75; 		/* Az állásjelzés offsete a DP adatbázisban */								/**/
TMOK_ID[29][0] =1293;						/* TMOK azonosítója a táviratban = DP offset */								/**/			
ReteszesRTUIndex[29][0] = 323;			/* G front end		 */															/**/
...
/* 32. TMOK: 44-11 RTU G: Celldömölk 0355/18 PV							-----------------------  */								/**/
...
/* 33. TMOK: 82-14 RTU G: Kömlõd 099 PV							-----------------------  */								/**/
TMOKAllasjelzesOffsetek[33] = 22; 		/* Az állásjelzés offsete a DP adatbázisban */								/**/
TMOK_ID[33][0] =1303;						/* TMOK azonosítója a táviratban = DP offset */								/**/			
ReteszesRTUIndex[33][0] = 323;			/* G front end		 */															/**/
...
nReteszOffset[33] = 31;             /* DC parancs és DP állapot offsete, ha tartozik hozzá DC parancs*/

/* 34. TMOK: 30-13 RTU G: Polgárdi 0155/12_1,2 PV							-----------------------  */								/**/
TMOKAllasjelzesOffsetek[34] = 10; 		/* Az állásjelzés offsete a DP adatbázisban */								/**/
TMOK_ID[34][0] =1307;						/* TMOK azonosítója a táviratban = DP offset */								/**/			
ReteszesRTUIndex[34][0] = 323;			/* G front end		 */															/**/
...
nReteszOffset[34] = 32;             /* DC parancs és DP állapot offsete, ha tartozik hozzá DC parancs*/

/* 35. TMOK: 35-22 RTU G: Polgárdi 0155/12_1,2 PV							-----------------------  */								/**/
TMOKAllasjelzesOffsetek[35] = 674; 		/* Az állásjelzés offsete a DP adatbázisban */								/**/
TMOK_ID[35][0] =1308;						/* TMOK azonosítója a táviratban = DP offset */								/**/			
ReteszesRTUIndex[35][0] = 323;			/* G front end		 */															/**/
...
nReteszOffset[35] = 33;             /* DC parancs és DP állapot offsete, ha tartozik hozzá DC parancs*/

/* 36. TMOK: 91-20 RTU G: Pakod 2144/2  PV							-----------------------  */								/**/
TMOKAllasjelzesOffsetek[36] = 432; 		/* Az állásjelzés offsete a DP adatbázisban */								/**/
TMOK_ID[36][0] =1312;						/* TMOK azonosítója a táviratban = DP offset */								/**/			
ReteszesRTUIndex[36][0] = 323;			/* G front end		 */															/**/
...
nReteszOffset[36] = 34;             /* DC parancs és DP állapot offsete, ha tartozik hozzá DC parancs*/

/* 37. TMOK: 43-25 A redundancia, RTU: H front end -> Vasalja 055/16 PV erõmû  -----------------------*/								/*  */
TMOKAllasjelzesOffsetek[37] = 224; 		/* Az állásjelzés offsete a DP adatbázisban */								/**/
TMOK_ID[37][0] =9;   						/* TMOK azonosítója a táviratban = DP offset */								/**/															
ReteszesRTUIndex[37][0] = 324;			/* A redundancia */															/**/
...
nReteszOffset[37] = 35;             /* DC parancs és DP állapot offsete, ha tartozik hozzá DC parancs*/

/* 38. TMOK: 40-98 RTU G: Szombathely 0773/1_2  PV erõmûvek							-----------------------  */								/**/
TMOKAllasjelzesOffsetek[38] = 69; 		/* Az állásjelzés offsete a DP adatbázisban */								/**/
TMOK_ID[38][0] =1314;						/* TMOK azonosítója a táviratban = DP offset */								/**/			
ReteszesRTUIndex[38][0] = 323;			/* G front end		 */															/**/
...
nReteszOffset[38] = 36;             /* DC parancs és DP állapot offsete, ha tartozik hozzá DC parancs*/

/* 39. TMOK: 86-81 RTU G: Bársonyos 0240/2 PV erõmû						-----------------------  */								/**/
TMOKAllasjelzesOffsetek[39] = 32; 		/* Az állásjelzés offsete a DP adatbázisban */								/**/
TMOK_ID[39][0] =1317;						/* TMOK azonosítója a táviratban = DP offset */								/**/			
ReteszesRTUIndex[39][0] = 323;			/* G front end		 */															/**/
...
nReteszOffset[39] = 37;             /* DC parancs és DP állapot offsete, ha tartozik hozzá DC parancs*/

/* 40. TMOK: 54-24 A redundancia, RTU: H front end -> Királyszentistván 017/34,35,36 PV erõmû , -----------------------*/								/*  */
TMOKAllasjelzesOffsetek[40] = 79; 		/* Az állásjelzés offsete a DP adatbázisban */								/**/

TMOK_ID[40][0] =11;   						/* TMOK azonosítója a táviratban = DP offset */								/**/															
ReteszesRTUIndex[40][0] = 324;			/* A redundancia */															/**/

TMOK_ID[40][1] =1325;   						/* TMOK azonosítója a táviratban = DP offset */								/**/															
ReteszesRTUIndex[40][1] = 323;			/* G front end */															/**/
...
nReteszOffset[40] = 38;             /* DC parancs és DP állapot offsete, ha tartozik hozzá DC parancs*/

/* 41. TMOK: 40-68 A redundancia, RTU: H front end -> Rádóckölked PV erõmû  -----------------------*/								/*  */
TMOKAllasjelzesOffsetek[41] = 434; 		/* Az állásjelzés offsete a DP adatbázisban */								/**/
TMOK_ID[41][0] =12;   						/* TMOK azonosítója a táviratban = DP offset */								/**/															
ReteszesRTUIndex[41][0] = 324;			/* A redundancia */															/**/
...
nReteszOffset[41] = 39;             /* DC parancs és DP állapot offsete, ha tartozik hozzá DC parancs*/

/* 42. TMOK: 21-39 G front end -> Csolnok 078/3,4 PV erõmû  -----------------------*/								/*  */
TMOKAllasjelzesOffsetek[42] = 54; 		/* Az állásjelzés offsete a DP adatbázisban */								/**/
TMOK_ID[42][0] =1324;   						/* TMOK azonosítója a táviratban = DP offset */								/**/															
ReteszesRTUIndex[42][0] = 323;			/* G Front end */															/**/
...
nReteszOffset[42] = 40;             /* DC parancs és DP állapot offsete, ha tartozik hozzá DC parancs*/

/* 43. TMOK: 92-65 G front end -> Csesztreg 066/2,3 PV erõmû  -----------------------*/								/*  */
TMOKAllasjelzesOffsetek[43] = 45; 		/* Az állásjelzés offsete a DP adatbázisban */								/**/
TMOK_ID[43][0] =1330;   						/* TMOK azonosítója a táviratban = DP offset */								/**/															
ReteszesRTUIndex[43][0] = 323;			/* G Front end */															/**/
...
nReteszOffset[43] = 41;             /* DC parancs és DP állapot offsete, ha tartozik hozzá DC parancs*/

/* 44. TMOK: 45-23 G front end -> Lukácsháza 0107/3 PV erõmû  -----------------------*/								/*  */
TMOKAllasjelzesOffsetek[44] = 509; 		/* Az állásjelzés offsete a DP adatbázisban */								/**/
TMOK_ID[44][0] =1332;   						/* TMOK azonosítója a táviratban = DP offset */								/**/															
ReteszesRTUIndex[44][0] = 323;			/* G Front end */															/**/
...
nReteszOffset[44] = 42;             /* DC parancs és DP állapot offsete, ha tartozik hozzá DC parancs*/

/* 45. TMOK: 31-68 G front end ->  PV erõmû  -----------------------*/								/*  */
TMOKAllasjelzesOffsetek[45] = 707; 		/* Az állásjelzés offsete a DP adatbázisban */								/**/
TMOK_ID[45][0] =1333;   						/* TMOK azonosítója a táviratban = DP offset */								/**/															
ReteszesRTUIndex[45][0] = 323;			/* G Front end */															/**/
...
nReteszOffset[45] = 43;             /* DC parancs és DP állapot offsete, ha tartozik hozzá DC parancs*/

/* 46. TMOK: 34-96 G front end ->  34768 TMOK, Bakonycsernye PV erõmû  -----------------------*/								/*  */
TMOKAllasjelzesOffsetek[46] = 361; 		/* Az állásjelzés offsete a DP adatbázisban */								/**/
TMOK_ID[46][0] =1342;   						/* TMOK azonosítója a táviratban = DP offset */								/**/															
ReteszesRTUIndex[46][0] = 323;			/* G Front end */															/**/
...
nReteszOffset[46] = 44;             /* DC parancs és DP állapot offsete, ha tartozik hozzá DC parancs*/

/* 47. TMOK: 91-34 G front end -> 99134 - Boncodfölde 038/1 PV erõmû  -----------------------*/								/*  */
TMOKAllasjelzesOffsetek[47] = 573; 		/* Az állásjelzés offsete a DP adatbázisban */								/**/
TMOK_ID[47][0] =1347;   						/* TMOK azonosítója a táviratban = DP offset */								/**/															
ReteszesRTUIndex[47][0] = 323;			/* G Front end */															/**/
...
nReteszOffset[47] = 45;             /* DC parancs és DP állapot offsete, ha tartozik hozzá DC parancs*/

/* 48. TMOK: 71-70 G front end -> Nagyveleg 011/7 PV erõmû  -----------------------*/								/*  */
TMOKAllasjelzesOffsetek[48] = 77; 		/* Az állásjelzés offsete a DP adatbázisban */								/**/
TMOK_ID[48][0] =1348;   						/* TMOK azonosítója a táviratban = DP offset */								/**/															
ReteszesRTUIndex[48][0] = 323;			/* G Front end */															/**/
ReteszesTMOK_RTUNum[48] = 1;				/* Az adott indexû TMOK ennyi kábelköri állomnással kommunikál */			/**/
nReteszPar[48] = 1;                /* 1: tartozik hozzá DC parancs, 0: nem tartozik hozzá DC parancs */
nReteszOffset[48] = 46;             /* DC parancs és DP állapot offsete, ha tartozik hozzá DC parancs*/
 
/* 49. TMKK: Sárár Spirit  G front end -> Ikervár 0745/3 PV 									/**/
DP1 = fnReadDPData(174, 0, 0, 0, 0);   /* Ha mindkét leégazás BENT van */
DP2 = fnReadDPData(175, 0, 0, 0, 0);

if (DP1==2 && DP2==2)
  {
  fnWriteDPData(1252, 2, 0, 0, 0, 0);  
  }
else
  {
  fnWriteDPData(1252, 1, 0, 0, 0, 0);  
  }  
...
nReteszOffset[49] = 47;             /* DC parancs offsete, ha tartozik hozzá DC parancs*/

/* 50. TMOK: 82-44 G front end -> Bokod 02/23 PV erõmû  -----------------------*/								/*  */
TMOKAllasjelzesOffsetek[50] = 313; 		/* Az állásjelzés offsete a DP adatbázisban */								/**/
TMOK_ID[50][0] =1351;   						/* TMOK azonosítója a táviratban = DP offset */								/**/															
ReteszesRTUIndex[50][0] = 323;			/* G Front end */															/**/
ReteszesTMOK_RTUNum[50] = 1;				/* Az adott indexû TMOK ennyi kábelköri állomnással kommunikál */			/**/
nReteszPar[50] = 1;                /* 1: tartozik hozzá DC parancs, 0: nem tartozik hozzá DC parancs */
nReteszOffset[50] = 48;             /* DC parancs és DP állapot offsete, ha tartozik hozzá DC parancs*/

/* 51. TMOK: 81-46 G front end -> Dunaalmás 0633/PV PV erõmû  -----------------------*/								/*  */
TMOKAllasjelzesOffsetek[51] = 382; 		/* Az állásjelzés offsete a DP adatbázisban */								/**/
TMOK_ID[51][0] =1353;   						/* TMOK azonosítója a táviratban = DP offset */								/**/															
ReteszesRTUIndex[51][0] = 323;			/* G Front end */															/**/
